...
def buscar_dados_cliente(cliente_id):
    conn = create_connection()  # Tente criar a conexão
    if conn is None:  # Verifique se a conexão foi bem-sucedida
        print("Erro ao criar conexão. Encerrando a função.")
        return None
    ...
    try:
        ...
        close_connection(conn)  # Fecha a conexão corretamente
